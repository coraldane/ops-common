...
func (this *DesiredAgent) FillAttrs(osArch, workdir string) {
	...
	this.TarballUrl = fmt.Sprintf("%s/%s/%s", this.Tarball, osArch, this.TarballFilename)
	this.Md5Url = fmt.Sprintf("%s/%s/%s", this.Tarball, osArch, this.Md5Filename)
}
